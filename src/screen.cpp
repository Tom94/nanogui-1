...
#  error Did not select a graphics API!
    ...
    m_wants_color_management = m_display_primaries != 1 || m_display_transfer_function != 10 || m_display_sdr_level != 80.0f;
...
void Screen::move_window(const Vector2i &rel) {
    ...
    if (monitor_count > 0) {
        ...
        for (int i = 0; i < monitor_count; ++i) {
            ...
            glfwGetMonitorWorkarea(monitors[i], &monitor_work_pos[0], &monitor_work_pos[1], &monitor_work_size[0], &monitor_work_size[1]);
